//------------------------------------------------------------------------------
// <auto-generated>
//     這個程式碼是由範本產生。
//
//     對這個檔案進行手動變更可能導致您的應用程式產生未預期的行為。
//     如果重新產生程式碼，將會覆寫對這個檔案的手動變更。
// </auto-generated>
//------------------------------------------------------------------------------

namespace EMIC2.Models
{
    using System;
    using System.Collections.Generic;
    
    public partial class EEA2_RESIDENTS
    {
        public decimal RESIDENT_ID { get; set; }
        public string CITY_ID { get; set; }
        public string TOWN_ID { get; set; }
        public string VILLAGE_ID { get; set; }
        public string NEIGHBOR { get; set; }
        public string IDN { get; set; }
        public string RESIDENT_NAME { get; set; }
        public string SEX_TYPE { get; set; }
        public string RESIDENT_TYPE { get; set; }
        public string RESIDENT_ADDR { get; set; }
        public Nullable<decimal> BIRTH_YEAR { get; set; }
        public string BLOOD_TYPE { get; set; }
        public string TELEPHONE { get; set; }
        public string MEMO { get; set; }
        public string WGS84_PTS { get; set; }
        public string EMERGENCY_CONTACT { get; set; }
        public string EMERGENCY_TELEPHONE { get; set; }
        public string DEFAULT_SHELTER { get; set; }
        public string IS_LIVED { get; set; }
        public string MOBILE { get; set; }
        public string STREAM_CODE { get; set; }
        public string STREAM_NAME { get; set; }
        public string IS_SECURE { get; set; }
        public string IS_DEBRIS { get; set; }
        public string IS_FLOOD { get; set; }
        public string IS_TEMPORARY { get; set; }
        public string IS_FOREIGNER { get; set; }
        public string COUNTRY_ID { get; set; }
        public string IS_DELETE { get; set; }
        public string CREATED_USER { get; set; }
        public Nullable<System.DateTime> CREATED_TIME { get; set; }
        public string MODIFIED_USER { get; set; }
        public Nullable<System.DateTime> MODIFIED_TIME { get; set; }
        public Nullable<decimal> OLD_ID { get; set; }
    }
}
