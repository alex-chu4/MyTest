...
namespace EMIC2.Models.Helper
{
    public class CryptographyEx
    {
        ...
        public static string EncryptString(string plainText, string key)
        {
            ...
            using (MD5CryptoServiceProvider provider_MD5 = new MD5CryptoServiceProvider())
            {
                byte[] md5Byte = provider_MD5.ComputeHash(keyByte);

                // 產生加密實體
                using (var aesProvider = Aes.Create())
                using (ICryptoTransform aesEncrypt = aesProvider.CreateEncryptor(md5Byte, md5Byte))
                {

                    // output就是加密過後的結果
                    output = aesEncrypt.TransformFinalBlock(plainTextByte, 0, plainTextByte.Length);
                }
            }
            // 將加密後的位元組轉成16進制字串
            return output == null ? "" : BitConverter.ToString(output).Replace("-", string.Empty);
        }
        ...
        public static string DecryptString(string chipherText, string key)
        {
            ...
            using (MD5CryptoServiceProvider provider_MD5 = new MD5CryptoServiceProvider())
            {
                ...
                // 產生解密實體
                using (var aesProvider = Aes.Create())
                using (ICryptoTransform aesDecrypt = aesProvider.CreateDecryptor(md5Byte, md5Byte))
                {
