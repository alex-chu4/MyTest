...
namespace EMIC2.Models.Dao.ERA
{
    ...
    public class ERA2020123Dao : IERA2020123Dao
    {
        /// <summary>
        /// 查詢按鈕狀態TABLE FUNC (F1才需要，F1a不需要)
        /// </summary>
        /// <returns>(int type, string status)</returns>
        public ERA2Dto F1_QPER(ERA2Dto data)
        {
            ERA2Dto era2dto = new ERA2Dto();
            using (var conn = new SqlConnection(DBHelper.GetEMIC2DBConnection()))
            {
                string sql =
                    "select * from ERA2_020123_F1_QPER (default, @P_RPT_MAIN_ID, @P_EOC_ID, @P_PRJ_NO, @P_ORG_ID)";

                var parameters = new
                {
                    P_RPT_MAIN_ID = data.RPT_MAIN_ID,
                ...
                {
                    era2dto.DIAPLAY_TYPE_ID = item.DIAPLAY_TYPE_ID;
                    era2dto.ORG_CHECK = item.ORG_CHECK;
                }
        ...
        public IResult ERA2_UPD_F1F1a_S(ERA2Dto data)
        {
            ...
            using (var conn = new SqlConnection(DBHelper.GetEMIC2DBConnection()))
            {
                ...
                parameters.Add("@P_ORG_ID", data.ORG_ID, dbType: DbType.Int64, direction: ParameterDirection.Input);
                parameters.Add("@P_ORG_CHECK", data.ORG_CHECK, dbType: DbType.String, direction: ParameterDirection.Input);
            ...
            using (var conn = new SqlConnection(DBHelper.GetEMIC2DBConnection()))
            {
                try
                {
                   ...
                   @"select 
                    ...
                    era2dto.f1DaoResult.ForEach(e =>
                    {
                        e.CASE_TIME_TEXT = e.CASE_TIME?.ToString("yyyy-MM-dd HH:mm");
                    });
        ...
        public (List<HightWayDaoResult>, int MTMP_REC_ID, DateTime? SENDER_TIME) ERA2_MTMP_REC(ERA2Dto data)
        {
            ERA2Dto era2dto = new ERA2Dto();
            ...
            using (var conn = new SqlConnection(DBHelper.GetEMIC2DBConnection()))
            {
                try
                {
                    ...
                    era2dto.SENDER_TIME = rec.Select(e => e.SENDER_TIME).FirstOrDefault();
                ...
                return (era2dto.highwayDaoResult, recId, era2dto.SENDER_TIME);
            }
        }
